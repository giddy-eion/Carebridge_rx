...
export default function ClinicianLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (currentRole !== "clinician") router.replace("/");
  }, [currentRole, router]);

  if (currentRole !== "clinician" || !account) return null;

  const activeAlertCount = alerts.filter((a) => a.targetRole === "clinician" && a.status === "active").length;
