...
export default function PatientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (currentRole !== "patient") router.replace("/");
  }, [currentRole, router]);

  if (currentRole !== "patient" || !account) return null;

  const activeAlertCount = alerts.filter((a) => a.targetRole === "patient" && a.status === "active").length;
